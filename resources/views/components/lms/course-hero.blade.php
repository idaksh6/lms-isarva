...
<article {{ $attributes->merge(['class' => 'lms-course-hero']) }}>
    <div class="lms-course-hero-banner">
        <x-lms.illustration :variant="$motif" class="lms-banner--hero" />
        <div class="lms-course-hero-overlay" aria-hidden="true"></div>
        <div class="lms-course-hero-content">
            <div class="lms-course-hero-copy">
                <p class="lms-course-hero-eyebrow">{{ $course->code }}</p>
                @if ($active === 'show')
                    <h2 class="lms-course-hero-title">{{ $course->title }}</h2>
                @else
                    <h2 class="lms-course-hero-title">
                        <a href="{{ route('courses.show', $course) }}" class="text-white hover:text-white/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/60 rounded-sm">
                            {{ $course->title }}
                        </a>
                    </h2>
                @endif
                ...
                <div class="lms-course-hero-details">
                    <span class="lms-course-hero-pill">
