@if (config('lms.show_demo_credentials'))
    ...
    @endphp
    <div class="mt-6 rounded-xl border border-amber-200 bg-amber-50/80 p-4" x-data="{ open: false }">
        <button type="button" @click="open = !open"
                class="flex w-full items-center justify-between text-left text-sm font-semibold text-amber-900">
        ...
        <div x-show="open" x-cloak class="mt-3 space-y-3">
            <p class="text-xs text-amber-800">
                Password for all accounts: <code class="rounded bg-white px-1.5 py-0.5 font-mono text-amber-900">{{ $password }}</code>
            </p>

            <div class="overflow-hidden rounded-lg border border-amber-200/80 bg-white text-xs">
                <table class="w-full text-left">
                    <thead class="bg-amber-100/80 text-[10px] font-semibold uppercase tracking-wide text-amber-900">
                        <tr>
            ...
            <p class="text-[10px] text-amber-700">Click an email to fill the form. Run <code class="rounded bg-white px-1">php artisan db:seed</code> if accounts are missing.</p>
        </div>
    </div>
@endif
