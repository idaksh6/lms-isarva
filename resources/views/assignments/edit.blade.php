...
@section('title', 'Edit assignment — ' . $assignment->course->code)
@section('page_title', $assignment->course->title)

@section('content')
@php
    $course = $assignment->course->loadMissing('lecturer')->loadCount(['students', 'assignments']);
@endphp
<div class="lms-page-stack">
    <x-lms.course-hero :course="$course" active="assignment" />

    <div class="lms-form-card">
        <form method="POST" action="{{ route('assignments.update', $assignment) }}" enctype="multipart/form-data" id="assignment-edit-form" class="flex flex-col gap-6">
            @csrf
            @method('PATCH')
            ...
            <label class="lms-form-check">
                ...
                <span>
                    <span class="block text-sm font-semibold text-isarva-heading">Published — visible to students</span>
                    <span class="block text-xs text-isarva-muted">Uncheck to keep this assignment as a draft.</span>
                </span>
            </label>
        </form>
